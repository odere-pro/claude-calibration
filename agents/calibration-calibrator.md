...
2. **Dispatch every approved row through its bundle** at `<Bundles dir>/<row.bundle>/`:
   ...
   - **`scope: project`** rows you apply directly. **`scope: user`** rows you do *not* edit —
...
4. **Companion files.** If a `change` spec implies a companion file (e.g. "move the testing block
   ...
   file *and* trim `CLAUDE.md`), do both — that's one item. Never delete a file unless the row
   explicitly says to and is `risk: risky` *and* was approved.
...
7. Write `<run>/calibration-report-<ts>.md` with:
   ...
   - **Skipped** — approved rows that weren't applied, with the reason (a soft-fail verify is *not*
